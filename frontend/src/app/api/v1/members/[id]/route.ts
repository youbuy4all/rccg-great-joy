...
export async function GET(req: NextRequest, { params }:{ params: Promise<{id:string}> }) {
  return withAuth(req, async () => {
    ...
    const member = await prisma.member.findUnique({ where:{id}, include:{department:{select:{id:true,name:true}},houseFellowship:{select:{id:true,name:true}},user:{select:{id:true,email:true,role:true,lastLoginAt:true}}} });
...
export async function PATCH(req: NextRequest, { params }:{ params: Promise<{id:string}> }) {
  return withAuth(req, async () => {
    ...
    const member = await prisma.member.update({ where:{id}, data:clean, include:{department:{select:{id:true,name:true}},houseFellowship:{select:{id:true,name:true}}} });
    return ok(member);
