  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="font-serif font-bold text-gray-900 text-lg">Reports</h2>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handlePrint}
            className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl border border-gray-200 bg-white text-sm font-bold text-gray-600 hover:bg-gray-50 transition no-print">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              {finance.incomeBreakdown?.length > 0 && (
                <div className="space-y-3">
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-wide">Income by Category</p>
              ...
              {finance.expenseBreakdown?.length > 0 && (
                <div className="space-y-3">
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-wide">Expense by Category</p>
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-gray-50 rounded-xl p-4 text-center">
                <p className="text-2xl font-bold text-gray-900">{attendance.totalSessions}</p>
                <p className="text-[11px] font-bold text-gray-400 mt-0.5">Total Sessions</p>
              </div>
              <div className="bg-gray-50 rounded-xl p-4 text-center">
                <p className="text-2xl font-bold text-gray-900">{attendance.overallAvg}</p>
                <p className="text-[11px] font-bold text-gray-400 mt-0.5">Average Count</p>
              </div>
              <div className="bg-gray-50 rounded-xl p-4 text-center">
                <p className="text-2xl font-bold text-gray-900">{attendance.highestAttendance?.count ?? 0}</p>
                    ...
                    {Object.entries(attendance.byServiceType)
                      ...
                      .map(([type, data]: [string, any]) => (
                        <tr key={type} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="py-3 px-3 font-semibold text-gray-700">{data.count}</td>
                          <td className="py-3 px-3 font-semibold text-gray-700">{data.total.toLocaleString()}</td>
                          <td className="py-3 px-3 font-bold text-gray-900">{data.avg}</td>
                        </tr>
                      ))}
                  </tbody>
                </table>
