...
function StatCard({ icon, label, value, sub, color, darkColor }: {
  icon: React.ReactNode; label: string; value: string | number;
  sub?: string; color: string; darkColor: string;
}) {
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-5 flex gap-4 items-start">
    ...
    <div className="space-y-6">
      ...
      {finance && finance.totalIncome > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-5">
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-bold text-gray-700 dark:text-gray-300">Finance Health — {MONTHS[MONTH - 1]}</p>
            <span className={cn(
              "text-xs font-bold px-2.5 py-1 rounded-full",
              finance.netSurplus >= 0 ? "bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400" : "bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400"
            )}>
              ...
              {transactions.data.map(tx => (
                <div key={tx.id} className="flex items-center gap-3 px-5 py-3">
                  <span className={cn(
                    ...
                    tx.type === "INCOME" ? "bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400" : "bg-red-100 text-red-600 dark:bg-red-900/40 dark:text-red-400"
                  )}>
                    {tx.type}
                  </span>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
        {[
          ...
          { href: "/reports",     label: "Reports",     cls: "bg-pink-50   dark:bg-pink-900/20   text-pink-700   dark:text-pink-400"   },
        ].map(q => (
